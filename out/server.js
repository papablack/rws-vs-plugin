...
connection.onCompletion((textDocumentPosition) => {
    ...
    const parsedTemplate = (0, rwsIntegration_1.parseHtml)(lineHTML, fileDir);
    const _baseCompletions = [];
    if (!parsedTemplate) {
        return _baseCompletions;
    }
    const completed = [];
    for (const ci of parsedTemplate.classMetadata?.properties || []) {
        completed.push({
    ...
    for (const ci of parsedTemplate.classMetadata?.methods || []) {
        completed.push({
            label: `${ci.name}(${ci.arguments?.map((item) => `${item.name}: ${item.type}`).join(', ')}): ${ci.type}`,
            kind: ci.kind,
            data: {
                type: `${ci.type}`
            }
